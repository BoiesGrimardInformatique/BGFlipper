...
#include <furi_hal_power.h> // BGFlipper OS: pourcentage batterie sur l'accueil
...
static void desktop_clock_reconfigure(Desktop* desktop) {
    ...
    // BGFlipper OS: horloge + batterie toujours affichees sur l'accueil (ignore le reglage)
...
static void desktop_clock_draw_callback(Canvas* canvas, void* context) {
    ...
    char buffer[20];
    // BGFlipper OS: heure + pourcentage batterie
    snprintf(
        buffer,
        sizeof(buffer),
        "%02u:%02u %u%%",
        hour,
        desktop->clock.minute,
