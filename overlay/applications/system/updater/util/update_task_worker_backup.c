...
static bool update_task_post_update(UpdateTask* update_task) {
    ...
    do {
        ...
        if(update_task->state.groups & UpdateTaskStageGroupSplashscreen) {
            ...
            // BGFlipper OS: storage_common_copy n'écrase pas une destination existante.
            // On retire l'ancien slideshow pour garantir l'installation du nouveau
            // (sinon un ancien écran post-update persiste indéfiniment).
            storage_simply_remove(update_task->storage, INT_PATH(SLIDESHOW_FILE_NAME));
